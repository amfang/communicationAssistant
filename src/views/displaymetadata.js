...
//textarea内容变化,调用translator进行处理
var inputChange = require('../translator').actInputChange;
...
function updateTextScroll(){
  if (!scrolled){
    ...
    var elementTranscript = $('#home2 textarea').get(0);
...
function showTranscript(payload) {
  ...
  if (payload.msg.results && payload.msg.results[0] && payload.msg.results[0].final) {
    ...
    */
    ...
    $('#home2 textarea').val(output.transcript);
    //$('#resultsText').html(output.transcript);
      //alert("metadata:::" + output.transcript);
      // 这里实时返回语音录制转换的最终字符串，可以从这里进行翻译。
      //
    inputChange();
      //
      //
    ...
    //$('.nav-tabs .active').text() == 'Text' &&
    if (updateTranscript) {
      $('#home2 textarea').val(output.transcript + payload.text);
      //$('#resultsText').html(output.transcript + payload.text);
      //alert("else:::" + output.transcript + payload.text);
    }
  }
}
